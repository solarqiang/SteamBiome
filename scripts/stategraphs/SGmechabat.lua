...
local states =
{
    ...
    State{
        ...
        timeline = 
        {
            TimeEvent(1*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/bishop/voice") end ),
            TimeEvent(7*FRAMES, function(inst) inst:PushEvent("wingdown")  end ),
...
CommonStates.AddSleepStates(states,
{
    ...
    sleeptimeline = 
    {
        TimeEvent(23*FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/bee/beemine_rattle") end),
    },
...
CommonStates.AddCombatStates(states,
{
    attacktimeline = 
    {
        ...
        TimeEvent(8* FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/creatures/knight/attack") end),
    ...
    {
        TimeEvent(1*FRAMES, function(inst)  
        inst.SoundEmitter:PlaySound("dontstarve/creatures/knight/liedown")
        end),
...
return StateGraph("mechabat", states, events, "idle", actionhandlers)